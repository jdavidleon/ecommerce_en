...
{	
	...
	function __construct()
	{
	    ...
	    $this->url_user = urldecode(URL_BASE.'page/usuarios/informacion-personal/');
	...
	private function logIn()
	{
		...
		if ($loguear[1]->num_rows == 1) {
			...
			if ($fila['estado_usuario'] != 1) {
				parent::retornoError('ERROR_VALIDACION_USUARIOS_P_V',$this->url_origin);
			}
			if($fila['estado_usuario'] == 1){
				...
				if (isset($_COOKIE['addr_us_shp'])) {
					CRUD::falseDelete('usuarios_direcciones','id_usuario = ? AND tm_delete is NULL',['i',$_SESSION['id_usuario']]);	
